...
const Company: React.FC<CompanyProps> = ({ company, isLoading = false }) => {
  ...
  return (
    <article className="software-page" itemScope itemType="https://schema.org/SoftwareApplication">
        ...
        <div className="software-hero__main">
          <div className="software-hero__identity">
            ...
            <div className="software-hero__title-group">
              <h1 className="software-hero__name" itemProp="name">
                {isLoading ? <Skeleton width={250} /> : company?.name}
              </h1>
        ...
        {isLoading ? (
          <>
            {Array.from({ length: 4 }).map((_, idx) => (
              <div key={idx} className="software-info-bar__item">
                <span className="software-info-bar__label"><Skeleton width={80} /></span>
                <span className="software-info-bar__value"><Skeleton width={120} /></span>
              </div>
            ))}
          </>
        ) : (
      ...
      <section className="software-content">
        {isLoading ? (
          <div className="software-content__short">
            <h2 className="software-content__heading"><Skeleton width={300} /></h2>
            <div className="software-content__description">
              <Skeleton count={5} />
            </div>
          </div>
        ) : showLong ? (
      ...
      <section className="software-trust">
        <h2 className="software-trust__heading">
          {isLoading ? <Skeleton width={350} /> : `Pourquoi choisir ${company?.name} ?`}
        </h2>
        <div className="software-trust__grid">
          {isLoading ? (
            <>
              {Array.from({ length: 3 }).map((_, idx) => (
                <div key={idx} className="software-trust__card">
                  <div className="software-trust__icon"><Skeleton width={24} height={24} /></div>
                  <h3 className="software-trust__title"><Skeleton width={150} /></h3>
                  <p className="software-trust__text"><Skeleton count={2} /></p>
                </div>
              ))}
            </>
          ) : (
      ...
      {isLoading ? (
        <section className="software-tags">
          <h3 className="software-tags__heading"><Skeleton width={120} /></h3>
          <div className="software-tags__list">
            {Array.from({ length: 5 }).map((_, idx) => (
              <Skeleton key={idx} width={80} height={28} style={{ marginRight: 8 }} />
            ))}
          </div>
        </section>
      ) : company?.keywords && (
